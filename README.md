...
```bash
pip install tenseal pandas numpy matplotlib
```
...
python benchmark.py
```
Outputs `benchmark_results.json` and a summary table.

### 3. Generate performance charts
```bash
python plot_results.py
```
Outputs `benchmark_plots.png`.

### 4. Interactive demo
```bash
python demo.py
```
Interactively run any query over the full encrypted dataset.
...
- **HE is ~1000–2000× slower** than plaintext, which is the expected privacy cost.  
...
2. **Speed**: Plaintext is ~1,000–2,000× faster — this is the inherent cost of data confidentiality  
